...
PL.ACRONYMS = {
  ...
  'IAP':         'Identity-Aware Proxy — the generic category for reverse proxies that authenticate users via OIDC/SAML before forwarding to a backend. Cloudflare Access, AWS ALB authentication, FortiGate ZTNA, and Google IAP are all instances.',
  'ALB':         'Application Load Balancer — AWS\'s Layer-7 load balancer; supports built-in OIDC/Cognito authentication via listener rules',
  'EMS':         'Endpoint Management Server — in FortiGate ZTNA, the FortiClient EMS that issues device certificates, evaluates posture, and emits ZTNA tags to the FortiGate',
  'LCU':         'Load Balancer Capacity Unit — AWS\'s pricing metric for ALB (combines new connections, active connections, processed bytes, rule evaluations)',
  'VIP':         'Virtual IP — the public endpoint of a load balancer or gateway',
  'AV':          'Antivirus',
  'VPC':         'Virtual Private Cloud — AWS\'s private network construct',
  'MAU':         'Monthly Active Users — common pricing metric for IdPs and identity services',
  'FQDN':        'Fully Qualified Domain Name (e.g. wiki.corp.example.com)',
